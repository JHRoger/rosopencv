...
public:
        ...
        bool sendXYZ(short x,short y,short z);
private:
         int fd ;
         char tmpchar[8];
         const char *buffer;
...
bool Serialport::sendAngle(short _angle1,short _angle2)
{
    ...
    //tmpchar[2] = 0x06;                                  //the number of data bytes
        tmpchar[2] = _angle1 & 0x00ff;                      //第一个角度的低8位
        tmpchar[3] = _angle1 >> 8;                            //第一个角度的高8位
        tmpchar[4] = _angle2 & 0x00ff;                      //第二个角度的低8位
        tmpchar[5] = _angle2 >> 8;                          //第二个角度的高8位
    tmpchar[6] = (_angle1+_angle2) & 0x00ff;              //Check
        tmpchar[7] = 0xAA;                                  //End
        //tmpchar[7] = 0xFE;                                        //结束标志
    ...
    cout << "Check:" << hex << (int)tmpchar[6] << endl;
        return true;
}
...
bool Serialport::sendXYZ(short _x, short _y, short _z)
{
    char send_num=0;
        memset(tmpchar, 0x00, sizeof(tmpchar));    //对tempchar清零
        tmpchar[0] = 0xA5;                              //起始标志
        tmpchar[1] =  _x & 0x00ff;                      //x的低8位
        tmpchar[2] =  _x >> 8;                          //x的高8位
        tmpchar[3] =  _y & 0x00ff;                      //y的低8位
        tmpchar[4] =  _y >> 8;                          //y的高8位
        tmpchar[5] =  _z & 0x00ff;                       //z的低8位
        tmpchar[6] =  _z >> 8;                           //z的高8位
        tmpchar[7] = 0x5A ;                              //结束标志
